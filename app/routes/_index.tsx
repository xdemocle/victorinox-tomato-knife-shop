...
const BASE_PRICE = 9.50;
const DISCOUNT_PERCENTAGE = 0.4;

// List of developed countries (simplified for example)
const DEVELOPED_COUNTRIES = [
  'US', 'CA',  // North America
  'GB', 'DE', 'FR', 'IT', 'ES', 'NL', 'BE', 'CH', 'AT', 'SE', 'NO', 'DK', 'FI', // Europe
  'JP', 'KR', 'SG', 'AU', 'NZ'  // Asia-Pacific
];

// Eurozone countries
const EUROZONE_COUNTRIES = [
  'DE', 'FR', 'IT', 'ES', 'NL', 'BE', 'AT', 'FI', 'IE', 'PT', 'GR', 'SK', 'LV', 'LT', 'EE', 'SI', 'LU', 'MT', 'CY'
];
...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  const price = isDeveloped ? BASE_PRICE : BASE_PRICE * (1 - DISCOUNT_PERCENTAGE);
  
  // Set currency based on country
  const currency = (EUROZONE_COUNTRIES.includes(country) ? 'EUR' : 'USD') as Currency;
  ...
  const randomColor = colorVariants[Math.floor(Math.random() * colorVariants.length)].name;
...
export default function Index() {
  const { price, currency, locale, availableColor, isDiscounted } = useLoaderData<typeof loader>();
...
export function meta() {
  return [
    ...
    { 
      name: 'description',
      content: 'Experience the precision of Swiss craftsmanship with the Victorinox Tomato Knife. Perfect serration, ergonomic handle, and available in vibrant colors.'
    }
  ];
}
